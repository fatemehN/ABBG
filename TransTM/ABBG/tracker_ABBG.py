...
import time
##import AABG attack utils
import pysot_toolkit.trackers.ABBG_utils as auts
...
class Tracker(object):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        ##APPLY ABBG Attack
        im_adv = auts.ABBG_attack(self.net, x_crop_ori, self.templates_list, self.mask, 
                                  self.center_pos, s_x, image.shape[:2], self.window, self.window_penalty)

        # normalize
        x_crop = im_adv.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
