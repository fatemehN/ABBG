...
##Attack tool
import lib.test.tracker.ABBG_utils as auts
...
class ROMTrack(BaseTracker):
    ...
    def track(self, image, prev_box, info: dict = None):
        H, W, _ = image.shape
        self.frame_id += 1
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image, self.state, self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h)
        
        patch_arr = x_patch_arr.copy()
        ###Apply ABBG Attack
        x_patch_arr = auts.ABBG_attack(self.network, prev_box, x_patch_arr, resize_factor, self.params.search_size, self.template)
        ...
        if self.save_all_boxes:
            ...
            return {"target_bbox": self.state}, patch_arr, x_patch_arr
